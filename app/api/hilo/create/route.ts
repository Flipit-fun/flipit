...
export async function POST(req: NextRequest) {
    ...
    if (!choice || !['under', 'over'].includes(choice)) {
    ...
    const { data: game, error: insertError } = await supabase
        ...
        .insert({
            ...
            ip_hash: ipHash,
            game_type: 'hilo' // MUST ADD THIS BASED ON SCHEMA CHANGE
        })
        .select()
        .single();

    if (insertError || !game) {
        console.error('[hilo/create] DB error:', insertError);
