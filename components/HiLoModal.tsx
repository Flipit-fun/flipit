...
export default function HiLoModal({ onClose, onGameCreated }: HiLoModalProps) {
    const network = 'mainnet';
    const [amount, setAmount] = useState<number>(0.05);
    const [choice, setChoice] = useState<HiLoChoice>('under');
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch('/api/hilo/create', {
                method: 'POST',
    ...
    const payoutAmount = (amount * 2 * 0.95).toFixed(4); // User asked for 5% house edge on HiLo payout too
                ...
                <div className="mb-6">
                    ...
                    <div className="relative h-10 flex items-center bg-gray-200 rounded-lg overflow-hidden">
                        <input
                            ...
                            style={{
                                background: `linear-gradient(to right, #1A1A1A 0%, #1A1A1A ${Math.min(100, (amount - 0.05) / (Math.max(0.05, maxBet) - 0.05) * 100)}%, transparent ${Math.min(100, (amount - 0.05) / (Math.max(0.05, maxBet) - 0.05) * 100)}%, transparent 100%)`,
                            }}
                        />
                    </div>
                ...
                <div className="mb-5">
                    ...
                    >
                        <button
                            ...
                            style={
                                choice === 'under'
                                    ? { background: '#DC2626', color: '#FAFAF9' } // Red
                        ...
                        <button
                            ...
                            style={
                                choice === 'over'
                                    ? { background: '#2563EB', color: '#FAFAF9' } // Blue
                                    : { background: 'transparent', color: '#1A1A1A' }
                            }
                        >
                            Over 7
